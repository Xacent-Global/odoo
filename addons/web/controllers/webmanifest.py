...
class WebManifest(http.Controller):
        ...
        for module in module_ids:
            ...
            if data:
                shortcuts.append({
                    ...
                    'url': '/app?menu_id=%s' % data.mapped('res_id')[0],
    ...
    def _get_webmanifest(self):
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'Platform')
        manifest = {
            'name': web_app_name,
            'scope': '/app',
            'start_url': '/app',
    ...
    def _get_service_worker_content(self):
        """ Returns a ServiceWorker javascript file scoped for the backend (aka. '/app')
        """
    ...
    @http.route('/app/offline', type='http', auth='public', methods=['GET'], readonly=True)
    def offline(self):
